...
**编写技能就是将测试驱动开发（Test-Driven Development, TDD）应用于过程文档。**

**个人技能存储在特定于 agent 的目录中（Claude Code 使用 `~/.claude/skills`，Codex 使用 `~/.agents/skills/`）** 

你编写测试用例（带有 subagent 的压力场景），观察它们失败（基线行为），编写技能（文档），观察测试通过（agent 遵守规则），然后重构（填补漏洞）。

**核心原则：** 如果你没有观察到 agent 在没有技能的情况下失败，你就不知道技能是否教授了正确的内容。

**必要背景：** 在使用此技能之前，你必须理解 superpowers:test-driven-development。该技能定义了基本的 RED-GREEN-REFACTOR 循环。本技能将 TDD 适配到文档编写。

**官方指南：** 关于 Anthropic 官方的技能编写最佳实践，请参阅 anthropic-best-practices.md。该文档提供了额外的模式和指南，补充了本技能中以 TDD 为核心的方法。
...
| **最小代码** | 编写技能以解决这些特定的违规行为 |
| **观察通过** | 验证 agent 现在遵守规则 |
| **重构循环** | 发现新的合理化理由 → 堵塞 → 重新验证 |
...
- 机械性约束（如果可以通过 regex/validation 强制执行，就自动化它——将文档留给需要判断的情况）

## 技能类型

### 技术型（Technique）
具有具体步骤可遵循的方法（condition-based-waiting、root-cause-tracing）
...
**目的：** Claude 读取描述以决定为给定任务加载哪些技能。让它回答："我现在应该读取这个技能吗？"
...
**为什么这很重要：** 测试表明，当描述总结了技能的工作流时，Claude 可能会遵循描述而不是阅读完整的技能内容。一个描述说 "code review between tasks" 导致 Claude 只做一次审查，即使技能的流程图清楚地显示了两次审查（规范合规性然后代码质量）。

当描述改为仅 "Use when executing implementation plans with independent tasks"（无工作流总结）时，Claude 正确地阅读了流程图并遵循了两阶段审查过程。

**陷阱：** 总结工作流的描述会创建 Claude 会采用的捷径。技能主体变成了 Claude 跳过的文档。

```yaml
# ❌ 错误：总结工作流 - Claude 可能遵循此内容而不是阅读技能
...
- 描述*问题*（race conditions、不一致的行为）而不是*语言特定症状*（setTimeout、sleep）
...
# ✅ 正确：引用其他技能
始终使用 subagents（50-100x 上下文节省）。必需：使用 [other-skill-name] 了解工作流。
```
...
- ✅ 正确：`**必要背景：** 你必须理解 superpowers:systematic-debugging`
...
digraph when_flowchart {
    ...
    "需要展示信息?" -> "可能出错的决策?" [label="yes"];
    "可能出错的决策?" -> "小型内联流程图" [label="yes"];
    "可能出错的决策?" -> "使用 markdown" [label="no"];
}
```
...
| "这只是参考文档" | 参考文档可能有缺口、不清楚的部分。测试检索。 |
| "测试是过度的" | 未测试的技能有问题。总是如此。15 分钟测试节省数小时。 |
| "如果出现问题我会测试" | 问题 = agent 无法使用技能。在部署之前测试。 |
...
强制纪律的技能（如 TDD）需要抵御合理化。Agent 很聪明，在压力下会找到漏洞。

**心理学说明：** 理解说服技术为什么有效有助于你系统地应用它们。有关权威、承诺、稀缺、社会证明和统一原则的研究基础，请参阅 persuasion-principles.md（Cialdini, 2021; Meincke et al., 2025）。
...
这切断了整类"我在遵循精神"的合理化理由。

### 构建合理化表

从基线测试中捕获合理化理由（见下文测试部分）。Agent 提出的每个借口都进入表格：

```markdown
| 借口 | 现实 |
|--------|---------|
| "太简单不需要测试" | 简单代码会出错。测试需要 30 秒。 |
| "我会稍后测试" | 立即通过的测试什么也证明不了。 |
| "事后测试达到相同目标" | 事后测试 = "这做什么？" 事前测试 = "这应该做什么？" |
```
...
在没有技能的情况下用 subagent 运行压力场景。记录确切行为：
- 他们做出了什么选择？
- 他们使用了什么合理化理由（逐字记录）？
- 哪些压力触发了违规？

这就是"观察测试失败"——你必须看到 agent 在编写技能之前自然会做什么。

### GREEN：编写最小技能

编写解决这些特定合理化理由的技能。不要为假设情况添加额外内容。

在有技能的情况下运行相同场景。Agent 现在应该遵守。

### REFACTOR：关闭漏洞

Agent 发现了新的合理化理由？添加明确的应对措施。重新测试直到无懈可击。
