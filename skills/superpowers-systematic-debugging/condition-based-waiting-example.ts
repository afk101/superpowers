 ...
 * @param timeoutMs - 最大等待时间 (默认 5000ms)
 * @returns 返回解析为所有匹配事件的 Promise (当达到指定数量时)
 *
 * 示例:
  ...
  return new Promise((resolve, reject) => {
    ...
    const check = () => {
      ...
      if (matchingEvents.length >= count) {
        ...
        reject(
          new Error(
            `等待 ${count} 个 ${eventType} 事件超时,已等待 ${timeoutMs}ms (实际得到 ${matchingEvents.length} 个)`
          )
        );
      } else {
        setTimeout(check, 10);
