...
Bug 通常出现在调用栈的深处(在错误目录执行 git init、文件创建在错误位置、数据库使用错误路径打开)。你的直觉是在错误出现的地方进行修复,但这只是治标不治本。
...
2. WorktreeManager 使用空的 projectDir 调用
